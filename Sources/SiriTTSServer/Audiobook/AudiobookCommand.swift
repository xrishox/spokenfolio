...
/// `siri-tts-server audiobook …`: EPUB inspection and audiobook creation.
/// Dispatched from Entrypoint; the other executable modes stay hand-rolled.
struct AudiobookCommand: AsyncParsableCommand {
  static let configuration = CommandConfiguration(
    commandName: "siri-tts-server audiobook",
    abstract: "Create audiobooks from EPUB books using installed Siri voices.",
    subcommands: [Create.self, Chapters.self, ExportText.self, Voices.self, Verify.self, Bench.self],
...
struct BookArguments: ParsableArguments {
  ...
  func plan(config: AudiobookConfig? = nil) throws -> AudiobookPlan {
    let resolved = config ?? ((try? AudiobookConfig.load()) ?? AudiobookConfig())
    do {
      return try AudiobookPlanner.plan(
        book: try loadBook(), options: try planOptions(config: resolved))
  ...
  struct Chapters: ParsableCommand {
    ...
    func run() throws {
      ...
      for section in loaded.sections {
        ...
        let index = String(section.spineIndex).leftPadded(to: 5)
  ...
  struct Voices: ParsableCommand {
    ...
    func run() throws {
      ...
      guard !assets.isEmpty else {
        throw CLIFailure(
          message:
            "no compatible Siri voices installed — download one in System Settings and run 'siri-tts-server doctor'",
          exitCode: 69 /* EX_UNAVAILABLE */)
      }
      print("Installed Siri voices (\(assets.count)):")
      for asset in assets {
        let name = asset.displayName.padding(toLength: 24, withPad: " ", startingAt: 0)
        let lang = asset.language.padding(toLength: 8, withPad: " ", startingAt: 0)
        print("  \(name)\(lang)\(asset.voiceInfo.quality.padding(toLength: 10, withPad: " ", startingAt: 0))\(asset.id)")
      }
    }
  }
}
