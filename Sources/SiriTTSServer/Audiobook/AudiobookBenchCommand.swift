  ...
  struct Bench: AsyncParsableCommand {
    ...
    func run() async throws {
      ...
      guard !assets.isEmpty else {
        throw CLIFailure(message: "no compatible Siri voices installed", exitCode: 69)
      }
      let voiceID =
    ...
    private func selectWorkload(corpus: [[[String]]]) throws -> [BenchUnit] {
      ...
      case .saturated:
        ...
        guard !all.isEmpty else {
          throw CLIFailure(message: "the book produced no narratable units", exitCode: 65)
        }
      ...
      case .chaptersSeq, .chaptersGlobal:
        ...
        guard chaptersTaken > 0 else {
          throw CLIFailure(message: "no substantial chapters found", exitCode: 65)
        }
        return selected
      }
    }
