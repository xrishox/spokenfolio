...
final class MenuBarApplication: NSObject, NSApplicationDelegate {
  ...
  private lazy var serverController: EmbeddedServerController = {
    ...
    controller.onEvent = { [weak self] event in self?.serverEvent(event) }
    return controller
  }()
  ...
  func applicationWillTerminate(_ notification: Notification) {
    ...
    serverController.stopImmediately()
  }
  ...
  private func serverEvent(_ event: EmbeddedServerController.Event) {
    switch event {
    case .starting:
      setState("Starting…")
    case .ready(let handle):
      endpointItem.title = "Copy http://localhost:\(handle.config.port)"
      switch handle.application.serverHealth.state {
      case .ready:
        setState("Ready — \(handle.application.ttsService.voiceCatalog.count) Siri voices")
  ...
  @objc private func restartServer() {
    Task { await serverController.restart() }
  }

  @objc private func runConnectionTest() {
    guard let port = try? ServerConfig.load().port,
      let voice = serverController.activeHandle?.application.ttsService.defaultVoice
    else { return }
    Task {
      do {
        try await ServerConnectionTester.run(port: port, voice: voice)
