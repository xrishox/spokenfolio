...
struct ServerConfig: Codable, Sendable {
  ...
  static func load(environment: [String: String] = ProcessInfo.processInfo.environment) throws
    -> Self
  {
    try AppConfig.load(environment: environment).server
  }

  mutating func applyEnvironment(_ environment: [String: String]) throws {
    if let host = environment["HTTP_HOST"], !host.isEmpty { self.host = host }
