...
| R18 | Some all-caps OOV names letter-spell (`SER`→"S-E-R", `ALAYAYA`→letters) while mixed-case twins speak correctly and other caps tokens (`TORNADO`, `LORD`, `GODRY`) speak as words; real-corpus appendix audio agrees across 2–3 ASR engines (incl. caps `ARYA`→"Uriah"). | Reproduced-private, single run + real-corpus multi-engine agreement |
| R19 | Full URLs are verbalized including scheme and query operators ("colon slash slash", "equals"); 15.7 s for one citation URL. | Reproduced-private, single run |
